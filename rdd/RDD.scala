...
import org.apache.spark.util.CshUtil
...
abstract class RDD[T: ClassTag](
  ...
  var indexMap = HashMap[String, Long]()
  def buildMap_person() = withScope {
    println("** start building index table")
    sc.runJob(this, buildLocalMap_person _)
    println("** completed building index table")
  }
  
  def buildLocalMap_person[U](iterator: Iterator[U]) = {
    var person_name = ""
    var person_actiontype = ""
    while(iterator.hasNext) {
      val data: Tuple2[Long, Array[String]] = iterator.next().asInstanceOf[Tuple2[Long, Array[String]]]
      // println("TYPE : " + data.getClass().getName()) --> scala.Tuple2
      if (data._2.size == 2)
      {
        person_name = data._2(0)
        person_actiontype = data._2(1)
        
        indexMap(person_name + person_actiontype) = data._1
        /**
        if(data._2(0) == "USERNAME")
        {
          last_name = data._2(1)
          indexMap(last_name) = data._1
        }
        else if(data._2(0) == "ACTION_TYPE")
        {
          last_actiontype = data._2(1)
          indexMap(last_name + last_actiontype) = data._1
        }
        **/
      }
      else
    ...
    println("LOCAL indexMap : " + indexMap)
  }
  

  def myMinimum() = withScope {
    // println("")
    // //println("*** " + this.getClass.getName + " ***")
    // println("\nmethods")
    // this.getClass.getDeclaredMethods foreach println
    
    // println("\nfields")
    // this.getClass.getDeclaredFields foreach println

    // println("")
    println(sc.runJob(this, findMin _).min)
   ...
   * my minimum method
   */
  def findMin[U](iterator: Iterator[U]): Long = {
    var local_minimum = 99999999L
    while (iterator.hasNext) {
      //println("TYPE:" + iterator.next().getClass().getName())
      local_minimum = Math.min(local_minimum, iterator.next().toString.toLong)
    }
    println("LOCAL MIN: "+local_minimum)
    local_minimum
  }
