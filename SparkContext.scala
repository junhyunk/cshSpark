...
//my class
//import org.apache.spark.input.PersonWritable
...
class SparkContext(config: SparkConf) extends Logging {
  ...
  def textFile_person(
      path: String,
      minPartitions: Int = defaultMinPartitions): RDD[Tuple2[Long, Array[String]]] = withScope {
    assertNotStopped()

    println("** textFile2() based on offset-text")
    
    hadoopFile(path, classOf[TextInputFormat], classOf[LongWritable], classOf[Text],
      minPartitions).map(pair => (pair._1.get, pair._2.toString.split(","))).setName(path)
  }
  /**
